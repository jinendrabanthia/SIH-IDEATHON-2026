 ...
 */
export function resolveSourceConflicts(facts) {
    if (!facts || facts.length === 0)
        return VerificationStatus.UNVERIFIED;
    if (facts.length === 1)
        return facts[0].verification_status;
    ...
    const verifiedGroups = Array.from(valueGroups.values()).filter(group => group.some(f => f.verification_status === VerificationStatus.VERIFIED));
 ...
 */
export function validateLLMNarration(text, validFactIds) {
