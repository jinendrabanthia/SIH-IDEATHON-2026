...
const exchangeRateCache = new TTLMemoryCache<any>(24 * 60 * 60 * 1000); // 24 hours
const holidayCache = new TTLMemoryCache<any>(30 * 24 * 60 * 60 * 1000); // 30 days
const countryCache = new TTLMemoryCache<any>(30 * 24 * 60 * 60 * 1000); // 30 days
const safetyCache = new TTLMemoryCache<any>(60 * 60 * 1000); // 1 hour
...
router.get('/holidays', validate(holidayQuerySchema, 'query'), async (req, res, next) => {
  try {
    const { countryCode, year } = req.query as unknown as { countryCode: string; year: number };
